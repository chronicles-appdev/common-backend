...
class  Query
{
    ...
    public function get_question_s($tt_id)
    {
        ...
        $stmt = $this->pdo->prepare("SELECT marking.id, questions.questionText, questions.optionA, questions.optionB, questions.optionC, questions.OptionD, questions.optionE, questions.images, questions.passage_id FROM marking  inner join questions  ON  marking.question_id = questions.id where  marking.test_taken_id = $tt_id  ORDER BY marking.id ASC");
    ...
    public function get_exam_details($tt_id)
    {
        ...
        $stmt = $this->pdo->prepare("SELECT years.year_name, subjects.subject_name FROM takeTest  inner join years  ON  years.id = takeTest.year_id  inner join subjects on subjects.id = takeTest.subject_id where  takeTest.id = $tt_id  ORDER BY takeTest.id ASC");
    ...
    public function get_history($student_id)
    {
        ...
        $stmt = $this->pdo->prepare("SELECT takeTest.id as id, takeTest.correctly_answ, takeTest.date_created, takeTest.num_question, subjects.subject_name  FROM takeTest inner join subjects on takeTest.subject_id = subjects.id where  takeTest.student_id = '$student_id' ORDER BY takeTest.id desc");
        $stmt->execute();
        $multi = $stmt->fetchAll(PDO::FETCH_OBJ);
